...
{{ range $method := .Service.Methods }}
    ...
    private function handle{{ $method.Desc.Name }}(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        ...
        switch (trim(strtolower(substr($header, 0, $i)))) {
            case 'application/json':
                $resp = $this->handle{{ $method.Desc.Name }}Json($ctx, $req);
                break;

            case 'application/protobuf':
                $resp = $this->handle{{ $method.Desc.Name }}Protobuf($ctx, $req);
                break;
    ...
    private function handle{{ $method.Desc.Name }}Json(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
    ...
    private function handle{{ $method.Desc.Name }}Protobuf(array $ctx, ServerRequestInterface $req): ResponseInterface
    {
        $ctx = Context::withMethodName($ctx, '{{ $method.Desc.Name }}');
