...
func (g *generator) generate(in *plugin.CodeGeneratorRequest) (*plugin.CodeGeneratorResponse, error) {
	...
	for _, file := range in.ProtoFile {
		...
		for _, svc := range file.Service {
			...
			serviceClientFile, err := g.generateServiceClient(file, svc)
			if err != nil {
				return nil, err
			}

			resp.File = append(resp.File, serviceInterfaceFile, serviceServerFile, serviceClientFile)
		}
	}
	...
	for namespace := range namespaces {
		...
		twirpClientFile, err := g.generateTwirpClient(namespace)
		if err != nil {
			return nil, err
		}

		resp.File = append(resp.File, protocolFile, serverFile, twirpClientFile)
	}
...
func (g *generator) generateServiceClient(file *descriptor.FileDescriptorProto, svc *descriptor.ServiceDescriptorProto) (*plugin.CodeGeneratorResponse_File, error) {
	data := &serviceDefinition{
		File:    file,
		Service: svc,
	}

	tpl, err := g.box.MustString("ServiceClient.php")
	if err != nil {
		return nil, err
	}
	...
	return &plugin.CodeGeneratorResponse_File{
		Name:    proto.String(fmt.Sprintf("%s/%sClient.php", PhpPath(file), PhpServiceName(svc))),
		Content: proto.String(tpl),
	}, nil
}
...
func (g *generator) generateTwirpClient(namespace string) (*plugin.CodeGeneratorResponse_File, error) {
	data := &twirpClientDefinition{
		Namespace:    namespace,
		TwirpVersion: TwirpVersion,
	}

	tpl, err := g.box.MustString("TwirpClient.php")
	if err != nil {
		return nil, err
	}
	...
	return &plugin.CodeGeneratorResponse_File{
		Name:    proto.String(fmt.Sprintf("%s/TwirpClient.php", PhpPathFromNamespace(namespace))),
		Content: proto.String(tpl),
	}, nil
}
